...
class ProductDetailsActivity : BaseActivity(), ProductDetailsContract.ProductDetailsView, OnImageClickListener {
    ...
    private fun onBuyNowBtnClick() {
        btn_buy_now.onClick {
            ...
            helper.insertData(obj)
        }
        Log.e("SqlData: ", helper.retrieveData().toString())
        makeToast(helper.retrieveData().toString())
    }
    ...
    private fun setResponseToProduct(detailsData: ProductDetailsData) {
        ...
        tv_product_price.text = "Rs." + when (detailsData.cost.toString().length) {
            4 -> StringBuilder(detailsData.cost.toString()).insert(1, ",")
            5 -> StringBuilder(detailsData.cost.toString()).insert(2, ",")
            else -> ""
        }
